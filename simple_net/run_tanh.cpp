...
/**
g++ Function.cpp libNet.cpp MyNet.cpp -o tanNet  $(pkg-config --cflags --libs opencv4)
 */
#include "libNet.h"
#include <ctime>
using namespace std;
...
//    net.initBias(s);
    ...
    get_input_label("./data/input_label_0-1_train_abc.xml", input, label, 1504);
    get_input_label("./data/input_label_0-1_test_abc.xml", test_input, test_label, 294);
...
//    get_input_label("./data/input_label_0-9_1000.xml", test_input, test_label, 200,800);
    ...
    timeStr<<now->tm_sec;
    std::string log_path = "./data/tanh_" + timeStr.str() + ".csv";
...
//    net.train1(input, label, loss_threshold);
     ...
     */
    //Save the model
    net.save("./models/model_tanh_1_0.001_20_100000_{26_50_2}_nobias_abc.xml");
    return 0;
}
